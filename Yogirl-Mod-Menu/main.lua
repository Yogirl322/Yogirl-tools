...
stuffThread = ScriptThread("Yogirl's Mod Menu " .. modVersion)

stuff = {}

function stuffThread:Run()
	...
	stuff.invinciblePlayerEnabled = false -- Start invincible.
	stuff.invincibleVehEnabled = false -- Start veh invincible.
	stuff.areaEnabled = false -- Start nearby peds join team.
	stuff.crazyVehiclesEnabled = false -- Start with vehicle mayhem.
	stuff.crazyPedsEnabled = false -- Start with crazy peds.
	stuff.noVehiclesEnabled = false -- Start with no traffic.
	stuff.speedBoostEnabled = false -- Start with speedboost.
	stuff.speedoEnabled = true -- Start with speedometer.
	stuff.speedoMoveEnabled = false -- Start with speedometer moveable.
	stuff.speedoMphEnabled = true -- Start with mph unit.
	...
	stuff.controllerHold = 0
	stuff.menu = {}
	stuff.lastIndex = 2
	
	stuff.currentVehicle = 1
	stuff.keyHoldCountRight = 0
	stuff.keyHoldCountLeft = 0
	
	stuff.timerAreaVehicles = 0
	stuff.timerCrazyVehicles = 0
	stuff.timerCrazyPeds = 0

	stuff.playerVel = 0, 0, 0
	stuff.playerSpeed = 0
	stuff.playerAvgSpeed = 0
	stuff.timerSpeed = 0
	stuff.speedTime = 0
	stuff.playerSpeedMax = 0.001
	
	if stuff.speedoMphEnabled then
		stuff.speedMult = 0.621371192
	else
		stuff.speedMult = 0
	end

	stuff.lastFrameCount = game.GetTime()
	stuff.frameDiff = 0
	...
	stuff.createMenuObject{
		text = "Spawn vehicle: " .. string.upper(string.sub(vehiclesList[stuff.currentVehicle][2], 1, 1)) .. string.lower(string.sub(vehiclesList[stuff.currentVehicle][2], 2,-1)),
		run = function()
			pos = LocalPlayer():GetOffsetVector(0,5,0)
			streaming.RequestModel(vehiclesList[stuff.currentVehicle][1])
			game.CreateVehicle(vehiclesList[stuff.currentVehicle][1], pos)
			streaming.ReleaseModel(vehiclesList[stuff.currentVehicle][1])
		end
	}
	...
	stuff.createMenuObject{
		...
		state = "Off",
		run = function()
			if stuff.invincibleVehEnabled then
				stuff.invincibleVehEnabled = false
				stuff.menu[stuff.lastIndex]["state"] = "Off"
			else
				stuff.invincibleVehEnabled = true
				if LocalPlayer():IsInVehicle() then
					LocalPlayer():GetVehicle():Fix()
					LocalPlayer():GetVehicle():SetDirtLevel(0)
				end
				stuff.menu[stuff.lastIndex]["state"] = "On"
			end
		end
	}
	...
	stuff.createMenuObject{
		...
		state = "Off",
		run = function() 
			if stuff.speedBoostEnabled then
				stuff.speedBoostEnabled = false
				stuff.menu[stuff.lastIndex]["state"] = "Off"
			else
				stuff.speedBoostEnabled = true
				stuff.menu[stuff.lastIndex]["state"] = "On"
			end
		end
	}
	...
	stuff.createMenuObject{
		...
		run = function()
			if stuff.invinciblePlayerEnabled then
				stuff.invinciblePlayerEnabled = false
				stuff.menu[stuff.lastIndex]["state"] = "Off"
				LocalPlayer():SetInvincible(false)
			else
				stuff.invinciblePlayerEnabled = true
				stuff.menu[stuff.lastIndex]["state"] = "On"
			end
		end
	}
	...
	stuff.createMenuObject{
		...
		state = "Off",
		run = function() 
			if stuff.areaEnabled then
				stuff.areaEnabled = false
				stuff.menu[stuff.lastIndex]["state"] = "Off"
			else
				stuff.areaEnabled = true
				stuff.menu[stuff.lastIndex]["state"] = "On"
			end
		end
	}
	...
	stuff.createMenuObject{
		...
		state = "Off",
		run = function()
			if stuff.crazyPedsEnabled then
				stuff.crazyPedsEnabled = false
				stuff.menu[stuff.lastIndex]["state"] = "Off"
			else
				stuff.crazyPedsEnabled = true
				stuff.menu[stuff.lastIndex]["state"] = "On"
			end
		end
	}
	...
	stuff.createMenuObject{
		...
		state = "Off",
		run = function()
			if stuff.crazyVehiclesEnabled then
				stuff.crazyVehiclesEnabled = false
				stuff.menu[stuff.lastIndex]["state"] = "Off"
			else
				stuff.crazyVehiclesEnabled = true
				stuff.menu[stuff.lastIndex]["state"] = "On"
			end
		end
	}
	...
	stuff.createMenuObject{
		...
		state = "Off",
		run = function()
			if stuff.noVehiclesEnabled then
				stuff.noVehiclesEnabled = false
				stuff.menu[stuff.lastIndex]["state"] = "Off"
			else
				stuff.noVehiclesEnabled = true
				stuff.menu[stuff.lastIndex]["state"] = "On"
			end
		end
	}
	...
	stuff.createMenuObject{
		...
		state = "On",
		run = function() 
			if stuff.speedoEnabled then
				stuff.speedoEnabled = false
				stuff.menu[stuff.lastIndex]["state"] = "Off"
			else
				stuff.speedoEnabled = true
				stuff.menu[stuff.lastIndex]["state"] = "On"
			end
		end
	}
	...
	stuff.createMenuObject{
		...
		state = "Off",
		run = function() 
			if stuff.speedoMoveEnabled then
				stuff.speedoMoveEnabled = false
				stuff.menu[stuff.lastIndex]["state"] = "Off"
			else
				stuff.speedoMoveEnabled = true
				stuff.menu[stuff.lastIndex]["state"] = "On"
			end
		end
	}
	...
	stuff.createMenuObject{
		...
		run = function()
			if stuff.speedMult ~= 0 then
				stuff.speedMult = 0
				stuff.menu[stuff.lastIndex]["state"] = "KP/H"
			else
				stuff.speedMult = 0.621371192
				stuff.menu[stuff.lastIndex]["state"] = "MP/H"
			end
		end
	}
	...
	while self:IsRunning() do
		stuff.frameDiff = game.GetTime() - stuff.lastFrameCount
		
		stuff.lastFrameCount = game.GetTime()

		stuff.updateMenu()
		
		if IsKeyDown(stuff.openMenuKey) or stuff.isControllerDown(BUTTON_R3) then
			if not stuff.drawMenu then
				stuff.drawMenu = true
				--natives.GAMEPLAY.SET_GAME_PAUSED(true)
				gui.BeepSelect()
			else
				stuff.drawMenu = false
		...
		if stuff.drawMenu and stuff.lastIndex == 2 then
			if IsKeyDown(KEY_NUMPAD6) or IsKeyDown(KEY_RIGHT) or IsKeyDown(KEY_D) or stuff.isControllerDown(BUTTON_LRIGHT) or stuff.isControllerDown(BUTTON_DRIGHT) then
				if stuff.keyHoldCountRight < 5 then
					stuff.keyHoldCountRight = stuff.keyHoldCountRight + 1
				end
				stuff.currentVehicle = stuff.currentVehicle + stuff.keyHoldCountRight
				if stuff.currentVehicle > #vehiclesList then
					stuff.currentVehicle = 1
				end
				stuff.menu[stuff.lastIndex]["text"] = "Spawn vehicle: " .. string.upper(string.sub(vehiclesList[stuff.currentVehicle][2], 1, 1)) .. string.lower(string.sub(vehiclesList[stuff.currentVehicle][2], 2,-1))
			else
				if stuff.keyHoldCountRight > 0 then
					stuff.keyHoldCountRight = stuff.keyHoldCountRight - 1
				end
			end
			if IsKeyDown(KEY_NUMPAD4) or IsKeyDown(KEY_LEFT) or IsKeyDown(KEY_A) or stuff.isControllerDown(BUTTON_LLEFT) or stuff.isControllerDown(BUTTON_DLEFT) then
				if stuff.keyHoldCountRight < 5 then
					stuff.keyHoldCountLeft = stuff.keyHoldCountLeft + 1
				end
				stuff.currentVehicle = stuff.currentVehicle - stuff.keyHoldCountLeft
				if stuff.currentVehicle < 1 then
					stuff.currentVehicle = #vehiclesList
				end
				stuff.menu[stuff.lastIndex]["text"] = "Spawn vehicle: " .. string.upper(string.sub(vehiclesList[stuff.currentVehicle][2], 1, 1)) .. string.lower(string.sub(vehiclesList[stuff.currentVehicle][2], 2,-1))
			else
				if stuff.keyHoldCountLeft > 0 then
					stuff.keyHoldCountLeft = stuff.keyHoldCountLeft - 1
				end
			end
		end
		...
		if stuff.invincibleVehEnabled then
			...
			if LocalPlayer():IsInVehicle() and natives.ENTITY.HAS_ENTITY_COLLIDED_WITH_ANYTHING(vehicle.ID) then
				...
				if math.floor(stuff.playerSpeed) == 0 then
		...
		if stuff.speedBoostEnabled then
			if (IsKeyDown(stuff.speedBoostKey) or stuff.isControllerDown(stuff.speedBoostButton)) and LocalPlayer():IsInVehicle() and not stuff.drawMenu then
				natives.ENTITY.SET_ENTITY_MAX_SPEED(LocalPlayer():GetVehicle().ID, 99999999)
				natives.VEHICLE.SET_VEHICLE_FORWARD_SPEED(LocalPlayer():GetVehicle().ID, stuff.playerSpeed/3.6 + 1)
			end
		end
		
		if stuff.speedoMoveEnabled and not stuff.drawMenu then
			if IsKeyDown(KEY_NUMPAD8) or stuff.isControllerDown(BUTTON_LUP) or stuff.isControllerDown(BUTTON_DUP) then
				stuff.speedoY = stuff.speedoY - 0.01
			end
			if IsKeyDown(KEY_NUMPAD2) or stuff.isControllerDown(BUTTON_LDOWN) or stuff.isControllerDown(BUTTON_DDOWN) then
				stuff.speedoY = stuff.speedoY + 0.01
			end
			if IsKeyDown(KEY_NUMPAD4) or stuff.isControllerDown(BUTTON_LLEFT) or stuff.isControllerDown(BUTTON_DLEFT) then
				stuff.speedoX = stuff.speedoX - 0.01
			end
			if IsKeyDown(KEY_NUMPAD6) or stuff.isControllerDown(BUTTON_LRIGHT) or stuff.isControllerDown(BUTTON_DRIGHT) then
				stuff.speedoX = stuff.speedoX + 0.01
			end
		end
		
		if stuff.speedoEnabled and LocalPlayer():IsInVehicle() then
				
			stuff.playerVel = LocalPlayer():GetVelocity()
			stuff.playerSpeed = 3.6 * math.sqrt((stuff.playerVel.x^2)+(stuff.playerVel.y^2)+(stuff.playerVel.z^2))
			
			stuff.timerSpeed = stuff.timerSpeed + stuff.frameDiff
		
			if stuff.timerSpeed >= 1000 then
				stuff.timerSpeed = stuff.timerSpeed - 1000
				stuff.speedTime = stuff.speedTime + 1
				stuff.playerAvgSpeed = stuff.playerAvgSpeed + stuff.playerSpeed
			end
			
			if stuff.playerSpeed > stuff.playerSpeedMax then
				stuff.playerSpeedMax = stuff.playerSpeed
			end
			if not stuff.drawMenu then
				local zeroStr = ""
				if stuff.playerSpeed < 10 then
					zeroStr = "00"
				elseif stuff.playerSpeed < 100 then
					zeroStr = "0"
				end
				gui.DrawRect( stuff.speedoX, stuff.speedoY, 0.13, 0.15, Color ( 20, 20, 20, 100 ) )
				if stuff.speedMult == 0 then
					if stuff.playerSpeed < 1000 then
						stuff.text(stuff.speedoX+0.01, stuff.speedoY+0.01, "KP/H: " .. zeroStr .. string.format("%.1f", stuff.playerSpeed), 0.7 + (stuff.playerSpeed/stuff.playerSpeedMax/10), Color(255 - (220*(stuff.playerSpeed/stuff.playerSpeedMax)),255, 100*(stuff.playerSpeed/stuff.playerSpeedMax)))
					else
						stuff.text(stuff.speedoX+0.01, stuff.speedoY+0.01, "KP/H: " .. math.floor(stuff.playerSpeed), 0.7 + (stuff.playerSpeed/stuff.playerSpeedMax/10), Color(255 - (220*(stuff.playerSpeed/stuff.playerSpeedMax)),255, 100*(stuff.playerSpeed/stuff.playerSpeedMax)))
					end
					stuff.text(stuff.speedoX+0.01, stuff.speedoY+0.07, "Highest speed: " .. string.format("%.1f", stuff.playerSpeedMax) .. " KP/H", 0.4, Color(255,180,0))
					stuff.text(stuff.speedoX+0.01, stuff.speedoY+0.09, "Average speed: " .. string.format("%.1f", (stuff.playerAvgSpeed/stuff.speedTime)) .. " KP/H", 0.4, Color(255,180,0))
					stuff.text(stuff.speedoX+0.01, stuff.speedoY+0.11, "Distance: " .. string.format("%.2f", stuff.playerAvgSpeed/60/60) .. " KM", 0.4, Color(255,180,0))
				else
					if stuff.playerSpeed < 1000 then
						stuff.text(stuff.speedoX+0.01, stuff.speedoY+0.01, "MP/H: " .. zeroStr .. string.format("%.1f", stuff.playerSpeed * stuff.speedMult), 0.7 + (stuff.playerSpeed/stuff.playerSpeedMax/10), Color(255 - (220*(stuff.playerSpeed/stuff.playerSpeedMax)),255, 100*(stuff.playerSpeed/stuff.playerSpeedMax)))
					else
						stuff.text(stuff.speedoX+0.01, stuff.speedoY+0.01, "MP/H: " .. math.floor(stuff.playerSpeed * stuff.speedMult), 0.7 + (stuff.playerSpeed/stuff.playerSpeedMax/10), Color(255 - (220*(stuff.playerSpeed/stuff.playerSpeedMax)),255, 100*(stuff.playerSpeed/stuff.playerSpeedMax)))
					end
					stuff.text(stuff.speedoX+0.01, stuff.speedoY+0.07, "Highest speed: " .. string.format("%.1f", stuff.playerSpeedMax * stuff.speedMult) .. " MP/H", 0.4, Color(255,180,0))
					stuff.text(stuff.speedoX+0.01, stuff.speedoY+0.09, "Average speed: " .. string.format("%.1f", (stuff.playerAvgSpeed/stuff.speedTime)* stuff.speedMult) .. " MP/H", 0.4, Color(255,180,0))
					stuff.text(stuff.speedoX+0.01, stuff.speedoY+0.11, "Distance: " .. string.format("%.2f", stuff.playerAvgSpeed/60/60* stuff.speedMult) .. " M", 0.4, Color(255,180,0))
				end
			end
		...
		if stuff.areaEnabled then
			stuff.timerAreaVehicles = stuff.timerAreaVehicles + stuff.frameDiff
			if stuff.timerAreaVehicles > 100 then
				stuff.timerAreaVehicles = 0
		...
		if stuff.crazyVehiclesEnabled then
			stuff.timerCrazyVehicles = stuff.timerCrazyVehicles + stuff.frameDiff
			if stuff.timerCrazyVehicles > 60 then
		...
		if stuff.crazyPedsEnabled then
			stuff.timerCrazyPeds = stuff.timerCrazyPeds + stuff.frameDiff
			if stuff.timerCrazyPeds > 60 then
				stuff.timerCrazyPeds = 0
...
function stuff.isControllerDown(b)
	if not stuff.drawMenu then
		if natives.CONTROLS.IS_CONTROL_JUST_PRESSED(2, b) then
			return true
		elseif natives.CONTROLS.IS_CONTROL_PRESSED(2, b) and stuff.controllerHold < 200 then 
			stuff.controllerHold = stuff.controllerHold + stuff.frameDiff
		elseif natives.CONTROLS.IS_CONTROL_PRESSED(2, b) and stuff.controllerHold >= 200 then
			stuff.controllerHold = stuff.controllerHold - stuff.frameDiff
			return true
		...
		if natives.CONTROLS.IS_DISABLED_CONTROL_JUST_PRESSED(2, b) then
			return true
		elseif natives.CONTROLS.IS_DISABLED_CONTROL_PRESSED(2, b) and stuff.controllerHold < 200 then 
			stuff.controllerHold = stuff.controllerHold + stuff.frameDiff
		elseif natives.CONTROLS.IS_DISABLED_CONTROL_PRESSED(2, b) and stuff.controllerHold >= 200 then
			stuff.controllerHold = stuff.controllerHold - stuff.frameDiff
			return true
...
function stuff.updateMenu()
	if stuff.drawMenu then
		natives.CONTROLS.DISABLE_ALL_CONTROL_ACTIONS(1)
		
		for i = 1, #stuff.menu do
			if i == stuff.lastIndex then
				if stuff.menu[i]["state"] == "Off" then
					gui.DrawRect( stuff.menuX + 0.002, stuff.menuY + (i/20) - 0.01, 0.3, 0.055, Color ( 150, 20, 0, 100 ) )
				else
					gui.DrawRect( stuff.menuX + 0.002, stuff.menuY + (i/20) - 0.01, 0.3, 0.055, Color ( 20, 150, 0, 100 ) )
				end
				stuff.text(stuff.menuX, stuff.menuY + (i/20) - 0.008, " " .. stuff.menu[i]["text"] .. stuff.menu[i]["state"], stuff.menu[i]["textScale"] + 0.3, stuff.menu[i]["textColorSelected"])
			else
				if i ~= 1 then
					if stuff.menu[i]["state"] == "Off" then
						gui.DrawRect( stuff.menuX - 0.01, stuff.menuY + (i/20) - 0.002, 0.27, 0.04, Color ( 50, 20, 0, 100 ) )
					else
						gui.DrawRect( stuff.menuX - 0.01, stuff.menuY + (i/20) - 0.002, 0.27, 0.04, Color ( 20, 50, 0, 100 ) )
					end
					stuff.text(stuff.menuX, stuff.menuY + (i/20), stuff.menu[i]["text"] .. stuff.menu[i]["state"], stuff.menu[i]["textScale"], stuff.menu[i]["textColor"])
				else
					stuff.text(stuff.menuX, stuff.menuY + (i/20) - 0.01, stuff.menu[i]["text"] .. stuff.menu[i]["state"], stuff.menu[i]["textScale"], stuff.menu[i]["textColor"])
				end
			end
		end
		if IsKeyDown(KEY_NUMPAD8) or IsKeyDown(KEY_W) or IsKeyDown(KEY_UP) or stuff.isControllerDown(BUTTON_LUP) or stuff.isControllerDown(BUTTON_DUP) then
			stuff.lastIndex = stuff.lastIndex - 1
			if stuff.lastIndex < 2 then
				stuff.lastIndex = #stuff.menu
			end
			gui.BeepNavUpDown()
		end
		
		if IsKeyDown(KEY_NUMPAD2) or IsKeyDown(KEY_S) or IsKeyDown(KEY_DOWN) or stuff.isControllerDown(BUTTON_LDOWN) or stuff.isControllerDown(BUTTON_DDOWN) then
			stuff.lastIndex = stuff.lastIndex + 1
			if stuff.lastIndex > #stuff.menu then
				stuff.lastIndex = 2
			end
			gui.BeepNavUpDown()
		end
		
		if IsKeyDown(KEY_NUMPAD5) or IsKeyDown(KEY_E) or IsKeyDown(KEY_RETURN) or stuff.isControllerDown(BUTTON_A) then
			stuff.menu[stuff.lastIndex]["run"]()
			gui.BeepSelect()
		end
		
		if IsKeyDown(KEY_BACK) or IsKeyDown(KEY_ESCAPE) or stuff.isControllerDown(BUTTON_B) then
			stuff.drawMenu = false
			gui.BeepBack()
		end
	end
...
function stuff.createMenuObject(t)
	local id = #stuff.menu + 1
	stuff.menu[id] = {}
	stuff.menu[id]["text"] = t.text
	stuff.menu[id]["textScale"] = t.textScale or 0.5
	stuff.menu[id]["textColor"] = t.textColor or Color(255, 180, 0)
	stuff.menu[id]["textColorSelected"] = t.textColorSelected or Color(140, 255, 0)
	stuff.menu[id]["state"] = t.state or ""
	stuff.menu[id]["run"] = t.run or false
end


function stuffThread:OnError()
	print("stuff Thread caused an error!")
	self:Reset()
end

stuffThread:Register()
